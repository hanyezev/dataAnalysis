...
[docs]class ARMA(tsa_model.TimeSeriesModel):
...
[docs]class ARIMA(ARMA):
    ...
    def __init__(self, endog, order, exog=None, dates=None, freq=None,
        ...
        p, d, q = order
        if d > 2:
            # TODO: to make more general, need to address the d == 2 stuff
...
[docs]class ARMAResults(tsa_model.TimeSeriesModelResults):
    """
    ...
    nobs : float
        ...
        用于拟合模型的观察数。 如果模型使用确切的最大似然来拟合，则等于观测值的总数n_totobs。
         如果使用条件最大似然拟合模型，则其等于n_totobs-k_ar。
    n_totobs : float
    ...
    sigma2 : float
        ...
        fit.
        残差的方差。 如果模型通过“ css”拟合，则sigma2 = ssr / nobs，其中ssr是残差平方的总和。
        如果模型通过'mle'拟合，则sigma2 = 1 / nob * sum（v ** 2 / F），其中v是单步预测误差，
        \F是预测误差方差。 有关适合度的不同定义，请参见`nobs`。
    """
    _cache = {}
...
[docs]    @Appender(_arma_results_predict)
...
[docs]    def summary2(self, title=None, alpha=.05, float_format="%.4f"):
        """
...
[docs]    @Appender(_arima_results_predict)
...
[docs]    @Appender(_arima_plot_predict)
    def plot_predict(self, start=None, end=None, exog=None, dynamic=False,
                     alpha=.05, plot_insample=True, ax=None):
        from statsmodels.graphics.utils import _import_mpl, create_mpl_ax
        _ = _import_mpl()
